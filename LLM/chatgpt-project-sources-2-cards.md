...
Твоя задача:
извлечь знания из файлов проекта (книга) → построить единый реестр знаний → создать Anki-карточки → подготовить экспорт.
...
3. Генерировать карточки без опоры на цитаты  
...
2. Использовать способности модели для:
   - анализа текста
   - классификации
   - структурирования  

НО:
→ не добавлять новые факты вне файлов
...
→ выдели логические темы (inferred)
...
"Подготовить ли copyable TXT для импорта в Anki с абсолютно всеми сгенерированными карточками?"

Не генерируй экспорт без подтверждения.

---

# EXPORT FORMAT (если пользователь согласен)

Подготовь экспорт в виде plain text UTF-8.
...
Используй нормализованные теги:
...
- не включать метаданные (Source/Quote) в карточки
- использовать только Q/A

Экранирование:

- если есть ; → обернуть поле в кавычки
- если есть " → заменить на ""
